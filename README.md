...
Available rules so far:  
  - `Between(min<int|float|string>, max<int|float|string>)` - Upper and lower bounds inclusive.  
  - `Email`  
  - `Equal(value<int|float|string>)`  
  - `GreaterOrEqual(value<int|float|string>)`  
  - `GreaterThan(value<int|float|string>)`  
  - `In(values<array>)`  
  - `LessOrEqual(value<int|float|string>)`  
  - `LessThan(value<int|float|string>)`  
